...
#Perform haplocall
angsd -b references/pre_list.filelist -doHaploCall 1 -doCounts 1 -out inter.files/1.haplo/${cur_chr}  -r ${cur_chr} -maxMis ${maxmis} -sites references/pobi.snps.by.chr/${cur_chr}.se.pobi.bim.chr.pos.txt

#Convert to tped
/storage/software/angsd-0.913-22/misc/haploToPlink inter.files/1.haplo/${cur_chr}.haplo.gz inter.files/2.tped/${cur_chr}

#Replace N's with 0's
eval 'sed 's/N/0/g' inter.files/2.tped/${cur_chr}.tped  > inter.files/3.tped.nto0/temp.${cur_chr}.tped'

#Copy the tfam file
eval 'cp inter.files/2.tped/${cur_chr}.tfam inter.files/3.tped.nto0/temp.${cur_chr}.tfam'
